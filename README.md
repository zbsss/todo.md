...
Optional agent metadata can also be stored in ticket frontmatter with `pr_link`, `branch`, `workspace`, and `assignee`. `assignee` supports Codex thread deep links such as `codex://threads/<thread-id>`.
